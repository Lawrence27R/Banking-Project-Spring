...
public class ClientController {
    ...
    @PutMapping("/{registrationNumber}/kyc-status")
    @PreAuthorize("hasRole('ROLE_SUPER_ADMIN')")
    public ResponseEntity<Client> updateClientKycStatus(
            @PathVariable Long registrationNumber, 
            @RequestBody ClientStatusUpdateDto statusUpdateDto) {
        Client updatedClient = clientService.updateClientKycStatus(registrationNumber, statusUpdateDto.getStatus());
        return ResponseEntity.ok(updatedClient);
    }
